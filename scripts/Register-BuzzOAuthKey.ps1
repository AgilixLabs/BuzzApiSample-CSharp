...
try {
    ...
    if ($response.StatusCode -eq 204) {
        ...
        Write-Host ""
        Write-Host "Key storage — choose one:"
        Write-Host "  Option A (PEM file):"
        Write-Host "    privateKeyPath = <path to your private_key.pem>"
        Write-Host ""
        Write-Host "  Option B (OS certificate store — no key file on disk):"
        Write-Host "    certThumbprint    = <certificate thumbprint>"
        Write-Host "    certStoreLocation = CurrentUser  # or LocalMachine for services"
    } else {
